...
type ValueController[T ~string] interface {
...
func NewValueController[T ~string](defaultT T, c *client.Client, logger *slog.Logger) ValueController[T] {
	return &vcImpl[T]{
		defaultT: defaultT,
		client:   c,
...
func (vc *vcImpl[T]) Set(ctx context.Context, key string, value T) error {
	err := client.WithRetriesVoid(ctx, vc.logger, func() error {
		fullKey := vc.buildPrefix(key)
		return vc.client.Set(fullKey, string(value))
	})
...
func (vc *vcImpl[T]) SetNX(ctx context.Context, key string, value T) error {
	err := client.WithRetriesVoid(ctx, vc.logger, func() error {
		fullKey := vc.buildPrefix(key)
		return vc.client.SetNX(fullKey, string(value))
	})
...
func (vc *vcImpl[T]) CompareAndSwap(ctx context.Context, key string, oldValue, newValue T) error {
	err := client.WithRetriesVoid(ctx, vc.logger, func() error {
		fullKey := vc.buildPrefix(key)
		return vc.client.CompareAndSwap(fullKey, string(oldValue), string(newValue))
	})

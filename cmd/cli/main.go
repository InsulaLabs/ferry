...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, "\n%s\n", color.YellowString("Records Operations:"))
	fmt.Fprintf(os.Stderr, "  %s %s\n", color.GreenString("records"), color.CyanString("list-groups"))
	fmt.Fprintf(os.Stderr, "    List all record group prefixes in use\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s\n", color.GreenString("records"), color.CyanString("delete-group"), color.CyanString("<prefix>"), color.CyanString("[--confirm]"))
	fmt.Fprintf(os.Stderr, "    %s Delete ALL records in a group via tombstones (background cleanup)\n", color.RedString("⚠"))

	fmt.Fprintf(os.Stderr, "  %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("get"), color.CyanString("<locator>"))
	fmt.Fprintf(os.Stderr, "    Get record by locator, show UUID and data\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("create"), color.CyanString("<locator>"), color.CyanString("[data]"))
	fmt.Fprintf(os.Stderr, "    Create new record with locator and optional data\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("delete"), color.CyanString("<uuid>"))
	fmt.Fprintf(os.Stderr, "    Delete record by UUID (marks for background cleanup)\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("list"), color.CyanString("[offset]"), color.CyanString("[limit]"))
	fmt.Fprintf(os.Stderr, "    List all records (default: offset=0, limit=100)\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("show"), color.CyanString("<locator>"))
	fmt.Fprintf(os.Stderr, "    Show full record details (UUID, locators, data)\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("set-data"), color.CyanString("<locator>"), color.CyanString("<data>"))
	fmt.Fprintf(os.Stderr, "    Set/update data for record\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("add-locator"), color.CyanString("<uuid>"), color.CyanString("<locator>"))
	fmt.Fprintf(os.Stderr, "    Add additional locator to record\n")

	fmt.Fprintf(os.Stderr, "  %s %s %s %s %s\n", color.GreenString("records"), color.CyanString("<prefix>"), color.CyanString("remove-locator"), color.CyanString("<uuid>"), color.CyanString("<locator>"))
	fmt.Fprintf(os.Stderr, "    Remove locator from record\n")
	...
	fmt.Fprintf(os.Stderr, "  \n")
	fmt.Fprintf(os.Stderr, "  # Records operations\n")
	fmt.Fprintf(os.Stderr, "  ferry records list-groups\n")
	fmt.Fprintf(os.Stderr, "  ferry records delete-group users        # Prompts for confirmation\n")
	fmt.Fprintf(os.Stderr, "  ferry --confirm records delete-group users  # No prompts\n")
	fmt.Fprintf(os.Stderr, "  ferry records users create \"email:alice@example.com\" '{\"name\":\"Alice\",\"age\":30}'\n")
	fmt.Fprintf(os.Stderr, "  ferry records users get \"email:alice@example.com\"\n")
	fmt.Fprintf(os.Stderr, "  ferry records users show \"email:alice@example.com\"\n")
	fmt.Fprintf(os.Stderr, "  ferry records users set-data \"email:alice@example.com\" '{\"name\":\"Alice\",\"age\":31}'\n")
	fmt.Fprintf(os.Stderr, "  ferry records users list 0 10\n")
	fmt.Fprintf(os.Stderr, "  ferry records users add-locator <uuid> \"id:12345\"\n")
	fmt.Fprintf(os.Stderr, "  ferry records users remove-locator <uuid> \"id:12345\"\n")
	fmt.Fprintf(os.Stderr, "  ferry records users delete <uuid>\n")
	fmt.Fprintf(os.Stderr, "  \n")
	...
	fmt.Fprintf(os.Stderr, "  - Records provide a higher-level abstraction with locators, transactions, and tombstone-based deletion\n")
...
func handleRecordsDelete(rc records.RecordController, args []string) {
	if len(args) != 1 {
		logger.Error("records delete: requires <uuid>")
		printUsage()
		os.Exit(1)
	}
...
func handleRecordsList(rc records.RecordController, args []string) {
	...
	fmt.Fprintf(os.Stdout, "%-38s %s\n", color.CyanString("UUID"), color.CyanString("Locators"))
	fmt.Fprintf(os.Stdout, "%-38s %s\n", strings.Repeat("-", 38), strings.Repeat("-", 40))

	for _, record := range activeRecords {
		locators := record.GetLocators()
		locatorStr := strings.Join(locators, ", ")
		if locatorStr == "" {
			locatorStr = color.YellowString("(no locators)")
		}
...
func handleRecordsListGroups(f *core.Ferry, args []string) {
	...
	fmt.Fprintf(os.Stdout, "\n%s\n", color.CyanString("Record Groups:"))
	for _, prefix := range prefixes {
		fmt.Fprintf(os.Stdout, "  %s\n", prefix)
	}
	fmt.Fprintf(os.Stdout, "\n%s %d\n", color.GreenString("Total:"), len(prefixes))
}
